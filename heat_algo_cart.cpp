...
void initial_value(const int n, const double dx, const double length, std::vector<double> &u)
{
  ...
  std::for_each(std::execution::par, ids.begin(), ids.end(), [u = u.data(), n, dx, length](auto idx) {
    auto [i, j] = idx;
...
double l2norm(const int n, const std::vector<double> &u, const unsigned int nsteps, const double dt, const double alpha, const double dx, const double length)
{
  ...
  return sqrt(std::transform_reduce(std::execution::par, ids.begin(), ids.end(), 0.0, std::plus<double>(), [u = u.data(), n, nsteps, dt, dx, alpha, length](auto idx)
  {
    auto [i, j] = idx;
    ...
    return (u[i+j*n] - answer) * (u[i+j*n] - answer);
  }));
}
