...
int main(int argc, char *argv[]) {
  ...
  double *u;
  double *u_tmp;
  hipMalloc(&u,     sizeof(double)*n*n);
  hipMalloc(&u_tmp, sizeof(double)*n*n);
  ...
  hipLaunchKernelGGL((initial_value), dim3(grid), dim3(block), 0, 0, n, dx, length, u);
  hipLaunchKernelGGL((zero), dim3(grid), dim3(block), 0, 0, n, u_tmp);
  ...
  for (int t = 0; t < nsteps; ++t) {
    ...
    if (t % 2 == 0)
      hipLaunchKernelGGL((solve), dim3(grid), dim3(block), 0, 0, n, alpha, dx, dt, u, u_tmp);
    else
      hipLaunchKernelGGL((solve), dim3(grid), dim3(block), 0, 0, n, alpha, dx, dt, u_tmp, u);
