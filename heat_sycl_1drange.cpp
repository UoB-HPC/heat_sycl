...
void initial_value(cl::sycl::queue& queue, const unsigned int n, const double dx, const double length, cl::sycl::buffer<double,1>& u) {
  ...
  queue.submit([&](cl::sycl::handler& cgh) {
    ...
    cgh.parallel_for<class initial_value_kernel>(cl::sycl::range<1>{n*n}, [=](cl::sycl::id<1> idx) {
      int i = idx[0] % n;
      int j = idx[0] / n;
...
void zero(cl::sycl::queue& queue, const unsigned int n, cl::sycl::buffer<double,1>& u) {
  ...
  queue.submit([&](cl::sycl::handler& cgh) {
    ...
    cgh.parallel_for<class zero_kernel>(cl::sycl::range<1>{n*n}, [=](cl::sycl::id<1> idx) {
      ua[idx] = 0.0;
    });
  });
...
void solve(cl::sycl::queue& queue, const unsigned int n, const double alpha, const double dx, const double dt, cl::sycl::buffer<double,1>& u_b, cl::sycl::buffer<double,1>& u_tmp_b) {
  ...
  queue.submit([&](cl::sycl::handler& cgh) {
    ...
    cgh.parallel_for<class solve_kernel>(cl::sycl::range<1>{n*n}, [=](cl::sycl::id<1> idx) {
      int j = idx[0] / n;
      int i = idx[0] % n;
